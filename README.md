...
- 过签（免 root）：静态去签 / 静态去签（高级·反自毁）/ NPatch 运行时 / IO 重定向（需原版 APK）4 种模式，一键重打包重签名；IO 重定向注入 MT 原生引擎（native IO 重定向 + PackageManager 签名伪造 + IBinder 代理），可在全新安装下对抗运行期强校验（如 Via 签名 KILL），目标 App 包名/证书运行期自动识别，支持超长包名
